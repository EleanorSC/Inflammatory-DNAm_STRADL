...
plot4 <- rbind(plot_brain_ageing_methylation,
  ...
  filter(brain_metric == "brain age" & estimate > 0) %>%
    filter(
      DNAm == "GHR"          | DNAm == "MMP.1.x" | DNAm == "CCL17" |DNAm == "MST1"    |DNAm==    "MMP1"     | DNAm ==    "MMP9"  | DNAm ==       "TPSB2"  |    
        DNAm == "CXCL10.x" | DNAm == "SIGLEC1"     | DNAm == "ICAM5" |DNAm == "SERPIND1"|DNAm==    "LGALS3BP" | DNAm ==    "VEGFA" | DNAm ==       "CRP"    |     
        DNAm == "SKR3"         | DNAm == "CCL18"       | DNAm == "THBS2" |DNAm == "NEP"     |DNAm==    "SELE"     | DNAm ==    "PIGR"  | DNAm ==       "MMP12"  |      
        DNAm == "STC1"         | DNAm == "FGF.21"      | DNAm == "ACY1"  |DNAm == "PRSS2"   |DNAm==    "IGFBP4"
    )
...
ggplot(plot4,
  ...
  geom_errorbar(
    ...
    position = position_dodge2(.9, padding = .6, preserve = "single"),
    #width = 0.25,
    size = 0.6
  ) +
  ...
  scale_fill_manual(values = c(
    colorspace::qualitative_hcl(26, palette = "Cold") 
  )) +
