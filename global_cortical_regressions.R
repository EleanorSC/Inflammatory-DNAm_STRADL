...
plot2 <- plot %>% filter(brain_metric == "global cortical surface area" |
...
plot2 %<>% mutate(brain_metric = factor(
  brain_metric,
  levels = c("global cortical volume",
...
pal <- colorspace::sequential_hcl(40, palette = "SunsetDark")

###### REORDER DNAm by effect size

plot3 <-  plot2 %>% filter(brain_metric == "global cortical volume" &
                             significance == "Yes")
...
x +
  ...
  scale_colour_manual(values = c("#7D1D67", "#841E6A", "#8C1E6D", "#931E6F", "#9A1F72", "#A12074", "#A82276", "#AF2477",
                                 ...
                                 "#FFB17F", "#FFB783", "#FFBD88", "#FFC28C", "#FFC891", "#FFCE96", "#FFD39B", "#FFD99F",
                                 "darkgrey")) +
